          ...
          <div className="single-entity">
            ...
            {Object.values(graphs).map((graph) => (
              <div
                ...
                key={graph.id}
                style={{
                  background: pickedGraph === graph.id ? "#000" : "#fff",
                  color: pickedGraph === graph.id ? "#fff" : "#000",
                }}
                className="single-entity-view"
              >
                {graph?.name || null}
              </div>
            ))}
          ...
          <div className="single-entity">
            ...
            {Object.values(entities).map((entity) => (
              <div
                ...
                style={{
                  background: pickedEntity === entity.id ? "#000" : "#fff",
                  color: pickedEntity === entity.id ? "#fff" : "#000",
                }}
                key={entity.id}
                className="single-entity-view"
              >
                {entity.name} ({entity.tag}){" "}
                {entity?.isAssigned ? "- Assigned" : ""}
              </div>
            ))}
          </div>
        </div>
        <div className="entities-setting">
          ...
          <div className="entity-info">
            ...
            {entities?.[pickedEntity]?.tag === "agent" && (
              <>
                <label>
                  Set Speed Multiple (Current:{" "}
                  {entities?.[pickedEntity]?.settings?.speed})
                </label>
                <input
                  value={speed}
                ...
                <button
                  onClick={() =>
                    updateEntity(pickedEntity, { settings: { speed: speed } })
                  }
                >
                  Update Speed
                </button>
            ...
            {entities?.[pickedEntity]?.tag !== "agent" && (
              <span>
                <FaInfo /> Only Agents can be bonded to a Behavior Graph!
              </span>
            )}
          </div>
        </div>
